...
export interface LLMResponse {
    summary: string;
    json: SpecOutput | ClarifyingQuestions | Partial<SpecOutput>;
    model_info: {
        model: string;
        provider: string;
        timestamp: Date;
...
export class LLMService {
    ...
    async generateClarifyingQuestions(
        ...
        try {
            ...
            return response.json as ClarifyingQuestions;
        } catch (error) {
    ...
    async refineSpec(
        ...
        try {
            ...
            return response.json as Partial<SpecOutput>;
        } catch (error) {
    ...
    private buildSpecPrompt(input: SpecInput, resolvedContext: ResolvedContext, mode: 'draft' | 'final'): string {
    ...
    private buildClarifyingQuestionsPrompt(input: SpecInput, resolvedContext: ResolvedContext): string {
    ...
    private buildRefinePrompt(originalSpec: SpecOutput, answers: Array<{ question: string; answer: string }>): string {
        const systemPrompt = templateEngine.render(TEMPLATE_NAMES.SYSTEM);
