...
export class LLMConfigManager {
    ...
    importConfiguration(id: string, configData: unknown): void {
    ...
    createFromTemplate(templateId: string, customizations?: Record<string, unknown>): LLMConfig {
    ...
    getConfigurationsRequiringApiKeys(): string[] {
        return Array.from(this.configs.entries())
            .filter(([, config]) => this.requiresApiKey(config))
            .map(([id]) => id);
    }
    ...
    validateAllConfigurations(): Array<{ id: string; valid: boolean; errors: string[] }> {
        return Array.from(this.configs.entries()).map(([id, config]) => {
            try {
                ...
                if (this.requiresApiKey(config) && !('api_key' in config && config.api_key)) {
