import { ResolvedContext, SpecInput, InputContext } from '@/types/schemas'
...
function calculateDiff(previous: ResolvedContext, current: ResolvedContext): Record<string, any> {
  const diff: Record<string, any> = {}
  
  // Simple field-by-field comparison
  for (const key in current) {
    if (JSON.stringify(previous[key as keyof ResolvedContext]) !== JSON.stringify(current[key as keyof ResolvedContext])) {
      diff[key] = {
...
export class ProjectContextService {
  ...
  async createProjectContext(
    ...
    const nextVersion = existingContexts.length > 0 
      ? Math.max(...existingContexts.map(c => c.version)) + 1 
      : 1
  ...
  async updateProjectContext(
    projectId: string,
    updates: Partial<ResolvedContext>,
    ...
    if (createNewVersion) {
      ...
      await dbService.updateProject(activeContext.id, activeContext as any)
  ...
  validateContext(context: ResolvedContext): {
    ...
    const incompleteStakeholders = context.stakeholders.filter(s => 
      !s.role || s.interests.length === 0
    )
